...
public class TalonSRXFactory {
    ...
    public static class Configuration {
        ...
        public double OPEN_LOOP_RAMP_LIMIT_RATE = 0.0;
        public double CLOSED_LOOP_RAMP_LIMIT_RATE = 0.0;
    }
    ...
    public static LazyTalonSRX createDefaultTalon(String name, int id) {
        return createTalon(name, id, kDefaultConfiguration);
    }

    public static LazyTalonSRX createSlaveTalon(String name, int id, int master_id) {
        final LazyTalonSRX talon = createTalon(name, id, kSlaveConfiguration);
        talon.set(ControlMode.Follower, master_id);
        return talon;
    }

    public static LazyTalonSRX createTalon(String name, int id, Configuration config) {
        LazyTalonSRX talon = new LazyTalonSRX(name, id);
        ...
        talon.configOpenloopRamp(config.OPEN_LOOP_RAMP_LIMIT_RATE, kTimeoutMs);
        talon.configClosedloopRamp(config.CLOSED_LOOP_RAMP_LIMIT_RATE, kTimeoutMs);
